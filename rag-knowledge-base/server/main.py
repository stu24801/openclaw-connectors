...
def _ask_writer_agent(article_id: str, owner_message: str):
    ...
    prompt = (
        ...
        f"   Header: X-Writer-Token: writer123，Content-Type: application/json\n"
        f"   Body: {{\"content\": \"你的回覆內容\"}}\n"
        f"   使用自然對話口吻，簡短即可。\n\n"
        f"2. 若需要修改文章內容，透過以下方式提交修改版（注意：必須用 multipart/form-data，不是 JSON）：\n"
        f"   curl -X POST https://rag.alex-stu24801.com/articles/{article_id}/revise \\\n"
        f"     -H 'X-Writer-Token: writer123' \\\n"
        f"     -F 'file=@/path/to/revised.md;type=text/markdown' \\\n"
        f"     -F 'note=修改說明'\n"
        f"   或用 exec 工具先把修改後內容存成暫存檔，再用 curl -F 上傳。\n\n"
